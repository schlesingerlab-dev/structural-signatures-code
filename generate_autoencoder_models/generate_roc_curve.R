...
ggplot(roc.total, aes( 1- specificity ,sensitivity , color = factor(comparison) )) + 
    ...
    geom_text(data = average.aucs[average.aucs$comparison=="gtex-archs-ss", ], aes(.55, .35, label = paste0("Train-GTeX::Predict-ARCHS4", "\n\t\t\tAUC: ", 
                        sprintf('%.2f', round(average_auc, digits=2 ) ) ), 
                            group = class), color = "black", size = 4.5)   +
    geom_text(data = average.aucs[average.aucs$comparison=="archs.predict.gtex.ss", ], aes(.55, .075, label = paste0("Train-ARCHS4::Predict-GTeX", "\n\t\t\tAUC: ", 
                        sprintf('%.2f', round(average_auc, digits=2 ) ) ), 
                            group = class), color = "black", size = 4.5) +
